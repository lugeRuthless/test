    ...
    public String uploadOpinionImage(MultipartFile file, HttpServletRequest request) {
        ...
        try {
            if (file != null) {
                ...
                //设置编译后文件存在路径
                String path = ClassUtils.getDefaultClassLoader().getResource("").getPath()+"static/upload/stayin/";
                path = java.net.URLDecoder.decode(path, "utf-8");

                //获取项目路径
                File directory = new File("src/main/resources/static/upload/stayin");
                ...
                data.put("src", "/upload/stayin/" + fileName);
                map.put("code", 0);
                map.put("msg", "上传成功！");
                map.put("data", data);
                return JSON.toJSONString(map);
